...
function render(data) {
  ...
  $("question").replaceChildren(document.createTextNode(data.maths.question)); for (const url of data.maths.questionImages || []) { const image = document.createElement("img"); image.src = url; image.alt = "Maths expression"; image.className = "math-expression"; $("question").append(" ", image); } $("answer").textContent = data.maths.answer; $("explanation").textContent = data.maths.explanation; $("math-source").textContent = `${data.maths.sourceName || "Generated locally"} ↗`; if (data.maths.source) setLink("math-source", data.maths.source);

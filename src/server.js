...
import { fallbackImage, historyFor, imageFor, japaneseFor } from "./upstream.js";
...
async function todayPayload() {
  ...
  const [historyResult, imageResult, japaneseResult] = await Promise.allSettled([historyFor(date, timeoutMs), imageFor(date, timeoutMs), japaneseFor(date, timeoutMs)]);
  const value = {
    ...base,
